...
class SenpaiGacha:
    ...
    async def jackpot(self, context):
        ...
        description = description + "\n\n**Current Jackpot Total: {} points\nTotal Number of Contributors: {}**".format(database_helper.get_jackpot(True)[0], len(contributors))
        ...
        while True:
            ...
            if high_streak is not None:
                ...
                if not 4 < next_quiz.hour < 13:
                    display_hour = (next_quiz - datetime.timedelta(hours=5)).hour % 12
                    if display_hour == 0:
                        display_hour = 12
            ...
            if not 4 < datetime.datetime.now().hour < 13: # generate quizzes only from 8am - 12am
                r = random.randint(1, 493) # generate random pokemon
